...
import monix.catnap.{CircuitBreaker, SchedulerEffect}
...
object CircuitBreakerSuite extends TestSuite[TestScheduler] {
  ...
  test(
    "should be stack safe for successful async tasks (inner protect calls)"
  ) { implicit s =>
  ...
  test("should be stack safe for successful immediate tasks (defer)") {
    implicit s =>
      val circuitBreaker = CircuitBreaker
        .of[IO](
          maxFailures = 5,
  ...
  test("complete workflow with failures and exponential backoff") {
    implicit s =>
      ...
      assertEquals(taskInError.unsafeToFuture().value, Some(Failure(dummy)))
      assertEquals(
        circuitBreaker.state.unsafeRunSync(),
      ...
      for (_ <- 0 until 30) {
        ...
        val delayedTask =
          circuitBreaker.protect(IO.sleep(1.second) *> IO.raiseError(dummy))
  ...
  private def awaitCloseSuccessfulTest(
      s: TestScheduler,
  ...
  test("canceled tasks in half-open state should open with old timeout") {
    implicit s =>
      import scala.concurrent.duration._
